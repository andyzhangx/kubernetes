...
func (as *availabilitySet) DetachDiskByName(diskName, diskURI string, nodeName types.NodeName) error {
	...
	if as.CloudProviderBackoff && shouldRetryHTTPRequest(resp, err) {
		glog.V(2).Infof("azureDisk - update(%s) backing off: vm(%s) detach disk(%s, %s), err: %v, disks: %v", nodeResourceGroup, vmName, diskName, diskURI, err, disks)
		retryErr := as.CreateOrUpdateVMWithRetry(nodeResourceGroup, vmName, newVM)
		if retryErr != nil {
			err = retryErr
			glog.V(2).Infof("azureDisk - update(%s) abort backoff: vm(%s) detach disk(%s, %s), err: %v, disks: %v", nodeResourceGroup, vmName, diskName, diskURI, err, disks)
		}
	}
	if err != nil {
